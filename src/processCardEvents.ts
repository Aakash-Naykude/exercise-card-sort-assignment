...
export const processCardEvents = (cardEvents: CardEvent[]): CardTransactionMapping => {
  ...
  cardEvents = cardEvents.sort()

  let object: { [CardId: string]: CardEvent[] } = {}

  for (var i = 0; i < cardEvents.length-1 ; i++) {
    if (
      cardEvents[i].cardId == cardEvents[i + 1].cardId &&
      ((cardEvents[i].type == 'RESERVATION' && cardEvents[i + 1].type == 'CANCELLATION') ||
        (cardEvents[i].type == 'RESERVATION' && cardEvents[i + 1].type == 'CONFIRMATION'))
    ) {
      var out: CardEvent[] = []
      out.push(cardEvents[i])
      out.push(cardEvents[i + 1])
      object[cardEvents[i].cardId] = out
    }
  }
